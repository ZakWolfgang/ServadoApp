...
export default function Header({ onAddMenuItemClick }) {
  const [showOptions, setShowOptions] = useState(false);
  const { toggleTheme } = useTheme();

  const navigate = useNavigate();

  const options = [{ name: "Add Menu Item", onClick: onAddMenuItemClick }];
  ...
  return (
    ...
    >
      {options.map(({ name, onClick }) => {
        return (
          <Option key={name} onClick={() => handleClick(onClick)}>
            {name}
          </Option>
        );
      })}
